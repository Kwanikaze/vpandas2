...
def train_VAE(data_train_full, data_test_full, compl_data_train_full, compl_data_test_full, wandb, args, norm_parameters):
    ...
    with torch.no_grad():
        ...
        if variational_params_train['qy'] == None:
            ...
            variational_params_train['qy'] = torch.ones((data_train_full.shape[0], 1)).cpu()
            recon_test['xobs'] = recon_test['xobs'].repeat((1,1,1)).cpu()
            variational_params_test['qy'] = torch.ones((data_test_full.shape[0], 1)).cpu()
        ...
        if args.num_samples == 1:
            ...
            if not args.mul_imp:
                ...
                recon_train, variational_params_train, latent_samples_train = model(data_train_filled_full.to(args.device), torch.tensor(M_sim_miss_train_full).to(args.device), test_mode=True, L=args.num_samples)
                ...
                if variational_params_train['qy'] == None:
                    recon_train['xobs'] = recon_train['xobs'].repeat((1,1,1,1)).cpu()
                    variational_params_train['qy'] = torch.ones((data_train_full.shape[0], 1)).cpu()
                    recon_test['xobs'] = recon_test['xobs'].repeat((1,1,1,1)).cpu()
                    variational_params_test['qy'] = torch.ones((data_test_full.shape[0], 1)).cpu()
